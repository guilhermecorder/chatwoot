...
// todo mundo escolhe (para si); admin pode escolher para a clínica
const onPaletteChip = () => openPalettePicker('panel');
</script>
    ...
    <div class="relative z-10 flex flex-col gap-3" style="color: #fff">
      <div class="flex items-start justify-between gap-3 flex-wrap">
        <div class="flex items-center gap-1.5 flex-wrap">
          ...
          <button
            ...
            :title="
              isAdmin
                ? 'Paleta desta página: só para você ou para a clínica — cor do dia, iMac G3, frutas da Apple ou salada de frutas'
                : 'Escolha as cores do seu painel: cor do dia (iMac G3), frutas da Apple ou salada de frutas — só você vê'
            "
            @click="onPaletteChip"
          >
            <span
            ...
            >
              · prévia</span>
            <span class="i-lucide-palette text-[10px] opacity-80" />
          </button>
          <slot name="chips" />
        </div>
        <div
      ...
      <div class="flex items-center gap-3 flex-wrap">
        ...
        <div class="min-w-0 flex-1">
          <h1
            class="text-2xl sm:text-[28px] font-bold leading-none tracking-tight"
            style="color: #fff"
          >
            {{ title }}
          </h1>
          <p
            v-if="subtitle"
            class="text-sm mt-1.5 max-w-2xl leading-relaxed"
