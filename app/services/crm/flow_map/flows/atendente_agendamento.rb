...
class Crm::FlowMap::Flows::AtendenteAgendamento
  FLOW = Crm::FlowMap::Flow.define(:atendente_agendamento) do # rubocop:disable Metrics/BlockLength
    name 'Atendente de Agendamento'
    ...
    node :move, 'Move o card para a coluna de agendado'
    ...
    live do |account|
      cfg = ai(account).dig('agents', 'atendente_agendamento') || {}
      events = Array(ai(account).dig('atendente_agendamento_state', 'events'))
      {
        enabled: agent_enabled?(account, 'atendente_agendamento') && Array(cfg['inbox_ids']).any?,
        last_run_at: last_at(events) || usage_last(account, 'atendente_agendamento'),
        counters: {
          ...
          'leituras (7 dias)' => usage_count(account, 'atendente_agendamento', 7.days.ago)
        }
      }
    end
  end
